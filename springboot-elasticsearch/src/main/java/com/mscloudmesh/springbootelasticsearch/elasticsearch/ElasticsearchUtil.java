package com.mscloudmesh.springbootelasticsearch.elasticsearch;

import com.alibaba.fastjson.JSONObject;

import com.mscloudmesh.springbootelasticsearch.model.Book;
...
//                System.out.println(JSON.toJSONString(resultMap));
     ...
     * @return
     */
    public static Double minAgg(String index,String indexType,String field) {
        AggregationBuilder agg= AggregationBuilders.min("aggMin").field(field);

        SearchResponse response=ElasticsearchUtil.client.prepareSearch(index).setTypes(indexType).addAggregation(agg).get();
     ...
     * @return
     */
    public static Double maxAgg(String index,String indexType,String field) {
        AggregationBuilder agg= AggregationBuilders.max("aggMax").field(field);

        SearchResponse response=ElasticsearchUtil.client.prepareSearch(index).setTypes(indexType)
     ...
     * @return
     */
    public static Double sumAgg(String index,String indexType,String field) {
     ...
     * @return
     */
    public static Double avgAgg(String index,String indexType,String field) {
     ...
     * @return
     */
    public static Stats stats(String indexName, String type, String field){
        AggregationBuilder agg=AggregationBuilders.stats("aggStats").field(field);
        SearchResponse response=client.prepareSearch(indexName).setTypes(type).addAggregation(agg).execute().actionGet();
     ...
     * @return
     */
    public static  ExtendedStats extendedStats(String indexName, String type, String field){
     ...
     * @return
     */
    public static double cardinality(String indexName, String type,String field){
        AggregationBuilder agg=AggregationBuilders.cardinality("cardinality").field(field);
        SearchResponse response=client.prepareSearch(indexName).setTypes(type).addAggregation(agg).get();
     ...
     * @return
     */
    public static  Percentiles percentiles(String indexName, String type,String field){
     ...
     * @return
     */
    public static double valueCount(String indexName, String type,String field){
     ...
     * @param fieldName
     */
    public static void multiGroup(String indexName,String esType,String fieldName){
        //指定索引和type
        SearchRequestBuilder builder = client.prepareSearch(indexName).setTypes(esType);
        //按team分组然后聚合，但是并没有指定聚合函数
        TermsAggregationBuilder teamAgg = AggregationBuilders.terms("book_count").field("name")
                ;



        //添加聚合器
        builder.addAggregation(teamAgg.subAggregation(AggregationBuilders.avg("priceAvg").field("price")));
        //触发
...
//        //取出聚合属性
        StringTerms terms = (StringTerms) aggMap.get("book_count");
        ...
        while (teamBucketIt .hasNext()) {
            ...
            System.out.println(team + " " + count+" "+avg.getValueAsString());
        }
    }
     ...
     * @param fieldName
     */
    public static void groupSearch(String indexName,String esType,String fieldName){
        //指定索引和type
        SearchRequestBuilder builder = client.prepareSearch(indexName).setTypes(esType);
        //按team分组然后聚合，但是并没有指定聚合函数
        TermsAggregationBuilder teamAgg = AggregationBuilders.terms("book_count").field("name");

        TermsAggregationBuilder agg = AggregationBuilders.terms("price_count").field(" price")
                .subAggregation(teamAgg);
